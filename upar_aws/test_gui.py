...
def selecionar_arquivo():
    ...
    if local_file:
        ...
        # Inicia o upload
        resultado = upload_arquivo_para_s3(local_file, bucket_name, s3_key, progress_callback)
        if isinstance(resultado, tuple):
            sucesso, erro_msg = resultado
        else:
            sucesso = resultado
            erro_msg = "Falha desconhecida no upload"

        if sucesso:
            messagebox.showinfo("Sucesso", "Arquivo enviado com sucesso!")
        else:
            messagebox.showerror("Erro", f"Falha ao enviar o arquivo: {erro_msg}")
