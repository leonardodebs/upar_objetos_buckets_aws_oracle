...
# Localiza o diretório base:
# - Quando empacotado como .exe (PyInstaller), usa o diretório do executável
...
class FileUploaderGUI:
    ...
    def _executar_upload(self, s3_key, bucket):
        try:
            env = _carregar_env()
            sucesso = upload_arquivo_para_s3(
                self.selected_file,
                bucket,
                s3_key,
                aws_config=env,
            ...
            self.window.after(0, self._finalizar_upload, sucesso)
        except Exception as e:
            # Em caso de crash na thread, avisa a finalização como erro
            self.window.after(0, self._finalizar_upload, False)
    ...
    def _finalizar_upload(self, sucesso: bool):
        self.btn_upload.state(["!disabled"])
        self.btn_selecionar.state(["!disabled"])
        ...
        if sucesso:
            ...
            messagebox.showerror(
                "Erro", "Falha no upload. Verifique as credenciais e tente novamente."
            )
